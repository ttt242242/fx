...
class MyFxTool
  ...
  def expect
    ...
    # every(1.day, 'get_data') do
      ...
      traning_data={:input => {0 =>@previous_data[:USDJPY].to_f/sum,1 =>@previous_data[:EURJPY].to_f/sum,2 =>@previous_data[:GBPJPY].to_f/sum,3 =>@previous_data[:CADJPY].to_f/sum,4 =>@previous_data[:AUDJPY].to_f/sum},:output => {11 =>@today_data[:USDJPY].to_f/sum2}} ;  #訓練データ(正規化したデータで
